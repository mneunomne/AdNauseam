...
vAPI.broadcastListener.add(request => {
  ...
  switch (request.what) {
    ...
    case 'notifications':
      utils.renderNotifications(request.notifications, 'vault');
      adjustHeight();
      createSlider();
      break;
  ...
  vAPI.messaging.send(
    ...
    vAPI.messaging.send(
      ...
      if (data.notifications && data.notifications.length)
        utils.renderNotifications(data.notifications, 'vault');
      adjustHeight();
    })
  })
...
const getHash = function (ad) {
  return ad.hash ? ad.hash : utils.computeHash(ad);
}
...
function parseAd(ad, data) {
  ...
  try {
    let network = ad.adNetwork ? ad.adNetwork : utils.parseHostname(ad.targetUrl);
  ...
  try {
    const domain = utils.parseDomain(ad.pageUrl);
...
function appendDetectedTo($detected, ad) {
  ...
  $('<a/>', {
    ...
    href: ad.pageUrl,
    text: utils.decodeEntities(ad.pageTitle),
    target: '_blank'
...
function appendTargetTo($target, ad, adset) {
  ...
  $('<a/>', {
    ...
    class: 'inspected-title',
    href: ad.targetUrl,
    text: utils.decodeEntities(ad.title),
    target: '_blank'
...
function updateMetaTarget($target, ad) {
  ...
  $target.find('#target-domain').text(utils.targetDomain(ad));
...
function appendPrivatePlaceHolder($pdiv, adset) {
  ...
  const $div = $('<div/>', {
    ...
    width: utils.rand(TEXT_MINW, TEXT_MAXW)
    ...
    $(".custom-menu li").click(function () {
      ...
      switch ($(this).attr("data-action")) {
        ...
        case "delete":
          ...
          // remove the adSet
          utils.arrayRemove(gAdSets, selectedAdSet);
...
function createAdSets(ads) {
  ...
  for (let i = 0; i < adsets.length; i++) {
    ...
    adsets[i].children.sort(utils.byField('-foundTs'));
  }
...
function repack() {
  ...
  utils.showVaultAlert(visible ? false : 'no ads found');
...
function createSlider(mode) {
  ...
  if (!gAds || !gAds.length) {
    computeStats();
    utils.showVaultAlert('no ads found');
    $('#loading-img').hide();
    return;
  }
  ...
  function computeMinDateFor(ads, min) {
    ...
    if (ads && ads.length) {
      ...
      ads.sort(utils.byField('-foundTs')); // or slice?
...
AdSet.prototype.nextPending = function () {
  ...
  ads.sort(utils.byField('-foundTs'));
...
$('#export').on('click', utils.exportToFile);
$('#import').on('click', utils.startImportFilePicker);
$('#importFilePicker').on('change', utils.handleImportAds);
$('#reset').on('click', utils.clearAds);
