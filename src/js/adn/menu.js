...
vAPI.broadcastListener.add(request => {
  ...
  switch (request.what) {
    ...
    case 'adVisited':
      ...
      break;
    case 'notifications':
      utils.renderNotifications(request.notifications);
      adjustBlockHeight();
      break;
...
const renderPage = function (json) {
  ...
  vAPI.messaging.send(
    ...
    vAPI.messaging.send(
      ...
      utils.renderNotifications(data.notifications)
...
const updateAd = function (ad) { // update class, title, counts
  ...
  if (verify(ad)) {
    ...
    // update the title
    $ad.descendants('.title').text(utils.decodeEntities(getTitle(ad)));

    // update the url
    $ad.descendants('cite').text(utils.targetDomain(ad));
...
const appendImageAd = function (ad, $items) {
  ...
  uDom(document.createElement('span'))
    .addClass('title')
    .text(ad.title ? utils.decodeEntities(ad.title) : "#" + ad.id)
    .appendTo($a);
...
const appendTextAd = function (ad, $items) {
  ...
  uDom(document.createElement('a'))
    ...
    .attr('href', ad.targetUrl)
    .addClass('title')
    .text(utils.decodeEntities(ad.title)).appendTo($h3);
